...
const MANAGEMENT_REGISTER_RESPONSE: &str = r#"{"ok":true,"result":{"Resources":[{"Path":"/status","Menu":"Example Rust Plugin","Description":"CPA exposes this menu resource under /v0/resource/plugins/{plugin-id}/status."}]}}"#;
...
fn make_http_response(request: &[u8]) -> String {
    ...
    let target = extract_json_string(&json, "URL")
        ...
        .unwrap_or_else(|| "/v0/resource/plugins/{plugin-id}/status".to_string());
    let body = format!(
