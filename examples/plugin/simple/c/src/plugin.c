...
static const char* MANAGEMENT_REGISTER_RESPONSE =
    ...
    "\"Menu\":\"Example C Plugin\",\"Description\":\"CPA exposes this menu resource under /v0/resource/plugins/{plugin-id}/status.\"}]}}";
...
static char* make_http_response(const uint8_t* request, size_t request_len) {
    ...
    char* target_escaped = json_escape(url != NULL ? url : (path == NULL ? "/v0/resource/plugins/{plugin-id}/status" : path));
