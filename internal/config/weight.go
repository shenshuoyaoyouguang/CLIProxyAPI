...
func (cfg *Config) ValidateCredentialWeights() error {
	...
	for index := range cfg.ZAIKey {
		if errValidate := ValidateCredentialWeight(cfg.ZAIKey[index].Weight); errValidate != nil {
			return fmt.Errorf("zai-api-key[%d].weight: %w", index, errValidate)
		}
	}
