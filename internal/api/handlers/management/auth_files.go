...
func (h *Handler) DownloadAuthFile(c *gin.Context) {
	...
	dirAbs, errAbs := filepath.Abs(authDir)
	if errAbs != nil {
		c.JSON(500, gin.H{"error": fmt.Sprintf("failed to resolve auth dir: %v", errAbs)})
		return
	}
	full := filepath.Clean(filepath.Join(dirAbs, name))
...
func (h *Handler) DeleteAuthFile(c *gin.Context) {
	...
	authDir, err = util.ResolveAuthDir(strings.TrimSpace(authDir))
	if err != nil || authDir == "" {
		c.JSON(http.StatusServiceUnavailable, gin.H{"error": "core auth manager unavailable"})
		return
	}
	dirAbs, errAbs := filepath.Abs(authDir)
	if errAbs != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": fmt.Sprintf("failed to resolve auth dir: %v", errAbs)})
		return
	}
	ctx := c.Request.Context()
	...
	if len(names) == 1 {
		if _, status, errDelete := h.deleteAuthFileByName(ctx, dirAbs, manager, names[0]); errDelete != nil {
	...
	for _, name := range names {
		deletedName, _, errDelete := h.deleteAuthFileByName(ctx, dirAbs, manager, name)
		if errDelete != nil {
...
func (h *Handler) deleteAuthFileByName(ctx context.Context, dirAbs string, manager *coreauth.Manager, name string) (string, int, error) {
	name = strings.TrimSpace(name)
	if isUnsafeAuthFileName(name) {
		return "", http.StatusBadRequest, fmt.Errorf("invalid name")
	}

	targetPath := filepath.Clean(filepath.Join(dirAbs, filepath.Base(name)))
	targetID := ""
...
func (h *Handler) upsertAuthRecord(ctx context.Context, auth *coreauth.Auth) error {
	manager, err := h.authManagerSnapshot()
	if h == nil || err != nil || manager == nil || auth == nil {
		return nil
	}
