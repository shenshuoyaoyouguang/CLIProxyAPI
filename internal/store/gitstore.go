	...
	if _, err := os.Stat(gitDir); errors.Is(err, fs.ErrNotExist) {
		...
		if _, errClone := git.PlainClone(repoDir, cloneOpts); errClone != nil {
			if errors.Is(errClone, transport.ErrEmptyRemoteRepository) {
				// RemoveAll can silently fail on Windows (released go-git handles plus
				// OS file locks), leaving a stale .git directory that makes the later
				// PlainInit fail because the directory is not empty. Try PlainInit
				// first; on failure fall back to PlainOpen to reuse the leftover
				// empty repository from the clone.
		...
		if errVerify := verifyRepositoryHead(repo); errVerify != nil {
			...
			// an open handle makes the directory rename fail with "Access is denied".
...
// renameWithRetry retries a directory rename with short backoff. On Windows,
// antivirus/indexer scans can briefly hold handles on a freshly corrupted .git,
// making the immediate rename fail with "Access is denied".
