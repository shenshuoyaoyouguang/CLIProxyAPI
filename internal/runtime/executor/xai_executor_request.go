...
func buildXAINamespaceDispatcherTool(tool gjson.Result) []byte {
		...
		for _, child := range nestedTools.Array() {
			...
			if params.Exists() && params.Raw != "" {
				...
				if rawParams != "" && rawParams != "{}" && rawParams != `{"type":"object","properties":{}}` {
					...
					if len(paramStr) > xaiDispatcherParamTruncateLimit {
						paramStr = paramStr[:xaiDispatcherParamTruncateLimit] + xaiDispatcherParamTruncateSuffix
					}
				}
			}
	...
	if len(fullDescription) > xaiDispatcherDescriptionTruncateLimit {
		fullDescription = fullDescription[:xaiDispatcherDescriptionTruncateLimit] + xaiDispatcherParamTruncateSuffix
	}
