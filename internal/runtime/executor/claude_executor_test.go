...
func TestApplyClaudeHeaders_LegacyModePreservesConfiguredUserAgentOverrideForClaudeClients(t *testing.T) {
	...
	// Confirmed clients keep the baseline platform tuple (MacOS/arm64) even when
	// the configured UA override replaces the User-Agent header.
...
func TestApplyClaudeHeaders_UnsetStabilizationUsesStableConfiguredOSArch(t *testing.T) {
	...
	// Unset stabilization falls back to the legacy path, where unconfirmed
	// clients report the local machine's OS/arch rather than the configured
	// software tuple.
