...
func (r *xaiNamespaceRestorer) restore(data []byte) []byte {
	...
	default:
		...
		if eventType == "response.completed" {
			// The restorer is per-request; once the response completes no further
			// dispatcher arguments arrive, so drop the id bookkeeping eagerly.
			clear(r.dispatcherItemIDs)
		}
		return data
	}
}
