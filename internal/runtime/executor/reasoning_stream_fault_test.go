...
func TestCodexStream_TransportEOFBeforeTerminalIsRequestScoped(t *testing.T) {
	...
	// HEAD's codex stream distinguishes a raw transport read failure (502) from a
	// stream that was read cleanly but ended before a terminal event (408). Both
	// are request-scoped; assert the read-failure shape for this transport EOF.
	if got := statusCodeFromTestError(t, streamErr); got != http.StatusBadGateway {
		t.Fatalf("status code = %d, want %d (transport read failure)", got, http.StatusBadGateway)
	}
	assertRequestScopedTestError(t, streamErr)
}
