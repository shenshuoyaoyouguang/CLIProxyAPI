...
func TestPrepareAntigravityGeminiReasoningReplayPayloadAppendsStaleThoughtSignatureWithoutNullParts(t *testing.T) {
	...
	item := []byte(`{"type":"thought_signature","contentIndex":8,"partIndex":3,"thoughtSignature":"sig-stale-text-value"}`)
	...
	if got := parts[1].Get("thoughtSignature").String(); got != "sig-stale-text-value" {
		t.Fatalf("thoughtSignature = %q, want sig-stale-text-value; body=%s", got, out)
	}
}
