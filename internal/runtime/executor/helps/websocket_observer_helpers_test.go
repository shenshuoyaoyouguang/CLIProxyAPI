...
func TestEmitWebSocketResponseEventRecoversFromObserverPanic(t *testing.T) {
	opts := cliproxyexecutor.Options{
		WebSocketResponseObserver: func(_ context.Context, _ cliproxyexecutor.WebSocketResponseEvent) {
			panic("observer boom")
		},
	}
	payload := []byte(`{"type":"codex.rate_limits"}`)
	// Must not panic: the helper is expected to contain observer panics so a
	// misbehaving observer cannot take down the executor's websocket loop.
	EmitWebSocketResponseEvent(context.Background(), opts, nil, "codex", "gpt-5.3-codex", payload)
}
