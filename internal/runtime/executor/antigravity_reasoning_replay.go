...
func antigravityReasoningReplayScopeFromRequest(ctx context.Context, modelName string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, payload []byte) antigravityReasoningReplayScope {
	...
	var scope antigravityReasoningReplayScope
	if sessionKey := antigravityReasoningReplayClientSessionKey(ctx, req, opts); sessionKey != "" {
		scope = antigravityReasoningReplayScope{modelName: modelName, sessionKey: sessionKey}
	...
	// Tenant isolation: every client-controlled session source (Session-Id header,
	// session_id / metadata.session_id / prompt_cache_key / content-hash fallback)
	// is namespaced by the caller API key so distinct callers cannot share replay
	// state. Server-issued "execution:" sessions keep their exact form.
