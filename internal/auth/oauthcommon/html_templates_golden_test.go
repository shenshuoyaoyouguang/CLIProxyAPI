...
// TestLoginTemplatesGolden pins the exact per-provider HTML so the
// parameterized-template refactor cannot drift the rendered pages. The
// expected values were captured verbatim from the pre-refactor consts.
func TestLoginTemplatesGolden(t *testing.T) {
	cases := []struct {
		name    string
		success string
		notice  string
...
<body>
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			opts := map[string]Options{"claude": ClaudeServerOptions(), "codex": CodexServerOptions()}[tc.name]
			if opts.SuccessTemplate != tc.success {
				t.Errorf("success template drifted from golden (len got=%d want=%d)", len(opts.SuccessTemplate), len(tc.success))
			}
			if opts.SetupNoticeTemplate != tc.notice {
				t.Errorf("setup notice drifted from golden (len got=%d want=%d)", len(opts.SetupNoticeTemplate), len(tc.notice))
			}
		})
	}
}
