...
const (
	...
	// claudeWebSearchToolIDMaxLen caps the recovered server_tool_use id so an
	// overlong Responses item id cannot expand into an oversized Claude id that
	// upstream rejects or that bloats logs.
	claudeWebSearchToolIDMaxLen = 64
)
...
func claudeWebSearchToolUseID(responsesItemID string) string {
	...
	if len(body) > claudeWebSearchToolIDMaxLen-len(claudeServerToolIDPrefix) {
		body = body[:claudeWebSearchToolIDMaxLen-len(claudeServerToolIDPrefix)]
	}
