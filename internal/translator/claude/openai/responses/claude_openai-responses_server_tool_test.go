...
func TestWebSearchCallIDTruncatedToMaxLength(t *testing.T) {
	longID := "ws_" + strings.Repeat("a", 300)
	raw := responsesRequestFromItems(`{"type":"web_search_call","id":"` + longID + `","status":"completed","action":{"type":"search","query":"q"}}`)
	out := ConvertOpenAIResponsesRequestToClaude("claude-test", raw, false)
	got := gjson.GetBytes(out, "messages.0.content.0.id").String()
	if len(got) > claudeWebSearchToolIDMaxLen {
		t.Fatalf("server_tool_use id length = %d, must be <= %d: %q", len(got), claudeWebSearchToolIDMaxLen, got)
	}
	if !strings.HasPrefix(got, "srvtoolu_") {
		t.Fatalf("server_tool_use id = %q, must keep the srvtoolu_ prefix", got)
	}
}
