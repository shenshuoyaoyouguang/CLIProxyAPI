...
// TestAntigravityTwinsGolden pins the exact wire output of every helper slated
// to move to the shared interactionscommon package (batch B-B). The values
// below were captured from the pre-refactor implementations and must not
// change when the bodies start delegating.

func TestGoldenAntigravityInlineDataPartJSON(t *testing.T) {
	cases := []struct {
		name  string
		input string
		want  string
...
func TestGoldenAntigravityFileDataPartJSON(t *testing.T) {
	cases := []struct {
		name  string
		input string
		want  string
...
func TestGoldenAntigravityInlineDataPartFromDataURL(t *testing.T) {
	cases := []struct {
		name string
		in   string
		want string
...
func TestGoldenInteractionsNativeAntigravityPart(t *testing.T) {
	cases := []struct {
		name  string
		input string
		want  string
		...
		{"functionResponse passthrough", `{"functionResponse":{"result":1}}`, `{"functionResponse":{"result":1}}`},
		{"camel inline", `{"inlineData":{"mimeType":"image/png","data":"QUJD"}}`, `{"inlineData":{"mimeType":"image/png","data":"QUJD"}}`},
		{"snake inline", `{"inline_data":{"mime_type":"image/png","data":"QUJD"}}`, `{"inlineData":{"mimeType":"image/png","data":"QUJD"}}`},
		{"camel file", `{"fileData":{"mimeType":"application/pdf","fileUri":"gs://b/f.pdf"}}`, `{"fileData":{"mimeType":"application/pdf","fileUri":"gs://b/f.pdf"}}`},
		{"unknown", `{"other":1}`, ``},
	}
	for _, tc := range cases {
...
func TestGoldenAntigravityContentRole(t *testing.T) {
...
func TestGoldenAntigravityInputAudioMimeType(t *testing.T) {
...
func TestGoldenAntigravityThinkingSummariesIncludeThoughts(t *testing.T) {
	cases := []struct {
		in            string
		wantV, wantOK bool
	}{
...
func TestGoldenAntigravityStreamAppends(t *testing.T) {
	...
	out = appendAntigravityInteractionsCompleted(out, st, "m1", root)
